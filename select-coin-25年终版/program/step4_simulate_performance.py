...
def _load_pivot_dict_spot():
    global _PIVOT_DICT_SPOT_CACHE
    if _PIVOT_DICT_SPOT_CACHE is None:
        _PIVOT_DICT_SPOT_CACHE = pd.read_pickle(get_file_path('data', 'market_pivot_spot.pkl'))
    return _PIVOT_DICT_SPOT_CACHE


def _load_pivot_dict_swap():
    global _PIVOT_DICT_SWAP_CACHE
    if _PIVOT_DICT_SWAP_CACHE is None:
        _PIVOT_DICT_SWAP_CACHE = pd.read_pickle(get_file_path('data', 'market_pivot_swap.pkl'))
...
def simulate_performance(conf, select_results, show_plot=True):
    ...
    pivot_dict_spot = _load_pivot_dict_spot()
    pivot_dict_swap = _load_pivot_dict_swap()
