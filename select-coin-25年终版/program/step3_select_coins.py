...
_ALL_FACTORS_CACHE = None


def _load_all_factors_df():
    global _ALL_FACTORS_CACHE
    if _ALL_FACTORS_CACHE is None:
        _ALL_FACTORS_CACHE = pd.read_pickle(get_file_path('data', 'cache', 'all_factors_df.pkl'))
    return _ALL_FACTORS_CACHE
...
def select_coins(conf: BacktestConfig, is_short=False):
    ...
    ** 策略选币 **
    ...
    # ====================================================================================================
    s = time.time()
    factor_df = _load_all_factors_df()
    ...
    3.4 后置过滤筛选（支持满仓模式）
    """
    if strategy.long_filter_list_post or strategy.short_filter_list_post:
        s = time.time()
        
        # 记录过滤前的状态（仅用于日志输出）
        long_count_before = len(factor_df[factor_df['方向'] == 1])
        short_count_before = len(factor_df[factor_df['方向'] == -1])
        
        # ⭐ 按时间点分组，记录每个时间点过滤前的权重总和
        long_weight_by_time = factor_df[factor_df['方向'] == 1].groupby('candle_begin_time')['target_alloc_ratio'].sum()
        short_weight_by_time = factor_df[factor_df['方向'] == -1].groupby('candle_begin_time')['target_alloc_ratio'].sum()
        
        # 执行后置过滤（返回多空分离的结果）
        long_df, short_df = strategy.filter_after_select(factor_df)
        
        # 根据配置决定是否重新分配仓位
        if strategy.reallocate_after_filter:
            # ⭐ 重新分配：按时间点分组，每个时间点独立重新分配
            if not long_df.empty:
                # 按时间点分组计算每个时间点剩余的币数
                coin_count_by_time = long_df.groupby('candle_begin_time')['symbol'].transform('size')
                # 每个币的新权重 = 该时间点过滤前的总权重 / 该时间点剩余的币数
                long_df['target_alloc_ratio'] = long_df['candle_begin_time'].map(long_weight_by_time) / coin_count_by_time
            
            if not short_df.empty:
                coin_count_by_time = short_df.groupby('candle_begin_time')['symbol'].transform('size')
                short_df['target_alloc_ratio'] = short_df['candle_begin_time'].map(short_weight_by_time) / coin_count_by_time
            
            weight_mode = '重新分配（满仓）'
        else:
            # 保持原权重：过滤掉的仓位闲置
            weight_mode = '保持原权重（部分仓位）'
        
        # 合并多空结果
        factor_df = pd.concat([long_df, short_df], ignore_index=True)
        
        # 打印后置过滤信息
        print(f'[选币] 后置过滤前：多头{long_count_before}个，空头{short_count_before}个')
        print(f'[选币] 后置过滤后：多头{len(long_df)}个，空头{len(short_df)}个')
        if long_count_before > 0 or short_count_before > 0:
            print(f'[选币] 权重分配模式：{weight_mode}')
            if strategy.reallocate_after_filter and not long_df.empty:
                sample_ratio = long_df["target_alloc_ratio"].iloc[0]
                print(f'[选币]   多头每币占比：{sample_ratio*100:.1f}%')
            if strategy.reallocate_after_filter and not short_df.empty:
                sample_ratio = short_df["target_alloc_ratio"].iloc[0]
                print(f'[选币]   空头每币占比：{sample_ratio*100:.1f}%')
...
def aggregate_select_results(conf):
    ...
    if not all_select_result_df[cond1 & cond2].empty:
        ...
        all_kline_df = _load_all_factors_df()
