...
from config import backtest_path, max_workers
...
def calc_factors(conf: BacktestConfig):
    ...
    if not all_factor_df_list:
        ...
        print('   可能原因：')
        print('   1. 数据准备阶段未读取到任何K线数据（请检查 config.py 中的 spot_path 和 swap_path 配置）。')
        print('   2. 所有币种的因子计算都失败了。')
        return
...
def trans_period_for_day(df, date_col='candle_begin_time'):
    ...
    agg_dict = {
        ...
        '是否交易': 'last',
        'is_spot': 'first',  # 添加 is_spot 列的聚合规则
    }
