...
# 导入配置、日志记录和路径处理的模块
from config import spot_path, swap_path, start_date, black_list, max_workers
...
def prepare_data(conf):
    ...
    if not spot_symbol_list:
        print(f'   ⚠️ 警告：在路径 {spot_path} 下未找到任何 *-USDT.csv 文件')
    ...
    if not swap_symbol_list:
        print(f'   ⚠️ 警告：在路径 {swap_path} 下未找到任何 *-USDT.csv 文件')

    if not spot_symbol_list and not swap_symbol_list:
        print('❌ 严重错误：没有读取到任何交易对数据！请检查 config.py 中的数据路径配置是否正确。')
        # 虽然这里报错，但为了不中断流程（可能会有后续处理），我们还是继续，但后续步骤肯定会失败
