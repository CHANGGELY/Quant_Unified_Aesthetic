...
**Objective**: Establish `Quant_Unified/基础库/通用选币回测框架` by merging `归档/select-coin-25年终版` features with `Quant_Unified`'s Long/Short capabilities, and applying deep localization (Chinese).
...
- [x] **Analyze Core Logic**:
    - Diff `归档/select-coin-25年终版/core/simulator.py` vs `Quant_Unified/策略仓库/一号择时策略/select-coin-feat-long_short_compose/core/simulator.py` (or best candidate) to identify Long/Short logic to preserve.
...
- [x] **Migrate Factors**:
    - Copy all factors from `归档/select-coin-25年终版/factors/` to `Quant_Unified/基础库/通用选币回测框架/因子库/`.
...
- [x] **Migrate Tools**:
    - Copy `归档/select-coin-25年终版/tools/` to `Quant_Unified/基础库/通用选币回测框架/工具箱/`.
- [x] **Fix Imports**:
...
- [x] **Migrate Program**:
    - Copy `归档/select-coin-25年终版/program/` to `Quant_Unified/基础库/通用选币回测框架/流程/`.
- [x] **Localization**:
