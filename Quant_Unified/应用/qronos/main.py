...
# 结构: Quant_Unified/应用/qronos/main.py
...
from config import MAX_DEVICES_PER_USER, DISABLE_AUTH
...
from service.basic_code import (
    ...
    extract_variables_from_coin_config, import_custom_framework
)
...
app.include_router(backtest_router, prefix=f"/{PREFIX}")
app.include_router(analysis_router, prefix=f"/{PREFIX}")
app.include_router(backtest_router, prefix="/flask")
app.include_router(analysis_router, prefix="/flask")
...
def get_user_devices(request: Request):
    ...
    try:
        # 免登录模式：返回空设备列表，避免前端报错
        logger.info("免登录模式：设备列表返回空")
...
def kick_device(device_id: str, google_code: str, request: Request):
    ...
    try:
        # 免登录模式：禁用设备管理
        logger.info("免登录模式：踢设备接口禁用")
        return ResponseModel.error(msg="免登录模式下不支持设备管理")
    except Exception as e:
...
@app.post(f"/{PREFIX}/basic_code/custom_framework")
def basic_code_custom_framework(request: CustomFrameworkRequest):
    """
    导入自定义框架
    
    :param request: 包含本地路径的请求体
    :return: 导入结果
    """
    logger.info(f"导入自定义框架: {request.path}")
    try:
        success, msg = import_custom_framework(request.path)
        if success:
            return ResponseModel.ok(msg=msg)
        else:
            return ResponseModel.error(msg=msg)
    except Exception as e:
        logger.error(f"导入自定义框架失败: {e}")
...
# ========== 框架路径更新 ==========
@app.post(f"/{PREFIX}/basic_code/update_path")
def basic_code_update_path(framework_id: str, new_path: str):
    """
    更新指定框架的本地路径
    """
    logger.info(f"更新框架路径: {framework_id} -> {new_path}")

    try:
        status = get_framework_status(framework_id)
        if not status:
            logger.error(f"框架不存在: {framework_id}")
            return ResponseModel.error(msg='框架不存在或未下载完成')

        if not Path(new_path).exists():
            logger.error(f"目标路径不存在: {new_path}")
            return ResponseModel.error(msg='目标路径不存在')

        ok = update_framework_status_and_path(framework_id, status.status, new_path)
        if not ok:
            return ResponseModel.error(msg='更新失败')

        logger.info("框架路径更新成功")
        return ResponseModel.ok()
    except Exception as e:
        logger.error(f"框架路径更新异常: {e}")
        return ResponseModel.error(msg=f'更新失败: {str(e)}')
...
def basic_code_operate(operate: BasicCodeOperateModel):
    ...
    operate_type = str(operate.type)
    if operate_type not in {"start", "stop", "restart", "log"}:
        logger.warning(f"不支持的操作类型: {operate.type}")
        return ResponseModel.error(msg="不支持的操作类型")

    operate_id = str(operate.framework_id if operate.pm_id is None else operate.pm_id)
    if not operate_id or operate_id.startswith("-"):
        return ResponseModel.error(msg="无效的 PM2 进程标识")
    if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9:_\-\.]*", operate_id):
        return ResponseModel.error(msg="无效的 PM2 进程标识")

    try:
        if operate_type in ["start", "stop", "restart"]:
            logger.info(f"执行PM2操作: {operate_type}")
            ...
            if not any([item['framework_id'] == operate.framework_id for item in data]):
                ...
                try:
                    start_result = subprocess.run(
                        ["pm2", "start", str(startup_config)],
                        env=env,
                        capture_output=True,
                        text=True,
                        timeout=60,
                    )
                    logger.info(f"PM2启动结果: {start_result.stdout}")
                    if start_result.stderr:
                        logger.warning(f"PM2启动警告: {start_result.stderr}")
                    if start_result.returncode != 0:
                        return ResponseModel.error(msg=f"PM2启动失败: {start_result.stderr or start_result.stdout}")
                    ...
                    if save_result.returncode != 0:
                        logger.warning(f"PM2保存失败: {save_result.stderr or save_result.stdout}")

                    return ResponseModel.ok(data="框架已启动并使用namespace配置")
                except Exception as e:
                ...
                try:
                    ...
                    if result.returncode != 0:
                        return ResponseModel.error(msg=f"PM2命令执行失败: {result.stderr or result.stdout}")
                    ...
                    logger.info(f"PM2操作已执行: {operate_type}")
                    return ResponseModel.ok(data=f"{operate_type} 命令已执行")
                except Exception as e:
                    logger.error(f"PM2操作异常: {e}")
                    return ResponseModel.error(msg=f"PM2命令执行失败: {str(e)}")

        elif operate_type == "log":
            logger.info(f"获取框架日志: {operate_id}, 行数: {operate.lines}")

            try:
                lines = int(operate.lines)
                if lines <= 0 or lines > 2000:
                    return ResponseModel.error(msg="lines 必须在 1~2000 之间")

                result = subprocess.run(
                    ["pm2", "logs", operate_id, "--lines", str(lines), "--nostream"],
                    env=env,
                    capture_output=True,
                    text=True,
                    timeout=30,
                ...
                if result.returncode != 0:
                    return ResponseModel.error(msg=f"日志获取失败: {result.stderr or result.stdout}")
...
# 挂载前端静态文件（本地开发支持）
spa_dist = Path(__file__).parent / "dist"
if spa_dist.exists():
    logger.info(f"挂载前端静态文件: {spa_dist}")
