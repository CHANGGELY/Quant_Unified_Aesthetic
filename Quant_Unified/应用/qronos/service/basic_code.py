...
from db.db_ops import save_framework_status, update_framework_status_and_path
from model.enum_kit import StatusEnum
from model.model import Pm2CfgModel
from service.command import create_pm2_cfg
from utils.constant import TMP_PATH, FRAMEWORK_ROOT_PATH
...
def import_custom_framework(path_str: str) -> Tuple[bool, str]:
    """
    导入自定义框架
    
    :param path_str: 本地框架路径
    ...
    try:
        ...
        framework_name = src_path.name
        # 使用人类可读的时间（精确到分钟）避免重名，若同分钟重复导入则追加递增后缀
        readable_ts = datetime.now().strftime("%Y%m%d-%H%M")
        base_name = f"{framework_name}_{readable_ts}"
        target_name = base_name
        target_path = FRAMEWORK_ROOT_PATH / target_name
        suffix_idx = 1
        ...
        # App 1: Custom Data Center (if exists)
        if (target_path / 'data_center.py').exists():
            dc_id = f"dc_{framework_id}"
        ...
        elif (target_path / 'realtime_data.py').exists():
            dc_id = f"dc_{framework_id}"
            cfg_dc = create_pm2_cfg(
        ...
        # App 2: Custom Strategy (if exists)
        if (target_path / 'startup.py').exists():
            st_id = f"st_{framework_id}"
        ...
        # App 3: Summary (if exists)
        if (target_path / 'summary_framework.py').exists():
             sm_id = f"sm_{framework_id}"
        ...
        if apps_config:
            ...
            config_path = target_path / 'startup.json'
            config_path.write_text(json.dumps(pm2_cfg.model_dump(), ensure_ascii=False, indent=2))
        
        # Register in DB
        save_framework_status(framework_id, framework_name, StatusEnum.FINISHED, 'custom', datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        update_framework_status_and_path(framework_id, StatusEnum.FINISHED, str(target_path))
