    ...
    >
      ...
      <template #footer>
        ...
        <Button
          ...
          autofocus
          :disabled="viewAddType === 'official' ? !selectFarmWorkId : !viewCustomFrameWorkPath"
        />
      </template>
    </Dialog>
...
const downloadFrameWorkAction = async () => {
  ...
  } else if (viewAddType.value === "custom") {
    if (viewCustomFrameWorkPath.value) {
      const res = await addCustomFrameWork(viewCustomFrameWorkPath.value);
      if (res.result === true) {
        isOpenAddFrameWorkDialog.value = false;
        toast.add({
          severity: "success",
