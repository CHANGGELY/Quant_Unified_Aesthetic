  ...
  >
      <div class="flex flex-col h-full">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-2">
          <Button
            size="small"
            :severity="viewLogFilter === '全部' ? 'primary' : 'secondary'"
            :outlined="viewLogFilter !== '全部'"
            label="全部"
            @click="() => { viewLogFilter = '全部'; applyLogFilter(); }"
          />
          <Button
            size="small"
            :severity="viewLogFilter === '仓位校正' ? 'primary' : 'secondary'"
            :outlined="viewLogFilter !== '仓位校正'"
            label="仓位校正"
            @click="() => { viewLogFilter = '仓位校正'; applyLogFilter(); }"
          />
          <Button
            size="small"
            :severity="viewLogFilter === '挂单健康' ? 'primary' : 'secondary'"
            :outlined="viewLogFilter !== '挂单健康'"
            label="挂单健康"
            @click="() => { viewLogFilter = '挂单健康'; applyLogFilter(); }"
          />
          <Button
            size="small"
            :severity="viewLogFilter === '网格重建' ? 'primary' : 'secondary'"
            :outlined="viewLogFilter !== '网格重建'"
            label="网格重建"
            @click="() => { viewLogFilter = '网格重建'; applyLogFilter(); }"
          />
          <Button
            size="small"
            :severity="viewLogFilter === 'PNL错误' ? 'primary' : 'secondary'"
            :outlined="viewLogFilter !== 'PNL错误'"
            label="PNL错误"
            @click="() => { viewLogFilter = 'PNL错误'; applyLogFilter(); }"
          />
        </div>
...
const viewRawLog = ref<string>("");
const viewLogFilter = ref<string>("全部");
...
const applyLogFilter = () => {
  const raw = viewRawLog.value || "";
  const filter = viewLogFilter.value;
  if (!raw) {
    viewDataLog.value = formatOutputLog("");
    return;
  }
  ...
  const filteredLines =
    filter && filter !== "全部"
      ...
      : lines;
  viewDataLog.value = formatOutputLog(filteredLines.join("\n"));
};
