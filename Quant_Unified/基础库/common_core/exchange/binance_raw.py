"""
common_core.exchange.binance_raw - 币安期货 REST API（原生 requests 版）

这个文件是干嘛的？
    这是“通用币安 REST API”的公共实现：
    - 完全不依赖 CCXT（一个第三方交易所聚合库），直接按币安官方文档拼请求
    - 支持 Demo Trading（测试网）与生产环境（实盘）
...
重要约定（唯一入口）：
    - 程序运行时只读取系统环境变量
    - `.env` 只是本地开发的“自动填充器”（不会覆盖你手动 export 的值）
...
def place_market_order(
    ...
    *,
    ...
    你可以把市价单理解成“我现在就要成交，价格先不纠结”：
    - 好处：几乎一定能成交（更符合“信号策略/高频策略”的需求）
    - 代价：会吃点差（bid/ask 差价）+ 可能有滑点（实际成交价比你看到的更差）

    参数解释：
        - reduce_only（只减仓）：True 表示只能平仓/减仓，不能加仓（合约风控常用）
        - position_side：对冲模式下要指定 LONG/SHORT；单向模式可不填
    """
    if quantity <= 0:
    ...
    if 调整后数量 <= 0:
        raise ValueError("quantity 太小，调整精度后变成 0")
    ...
    订单 = _请求("POST", "/fapi/v1/order", 参数, 需要签名=True)
    logger.info(f"市价单成功: {side} {调整后数量} {symbol}")
    return 订单
