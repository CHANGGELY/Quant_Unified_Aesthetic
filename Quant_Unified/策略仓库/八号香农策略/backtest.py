...
这个文件是干嘛的？
    你现在的实盘策略是“被动挂单吃波动”（Maker：挂单方，先把单挂在订单簿里等别人来撞），因为你 Maker 手续费为 0。
    所以回测如果还用“每分钟收盘瞬移调仓到 50:50”，就会严重失真（那等价于你能无限快成交，且通常只能用市价单）。

本回测的核心：
    1) 下单：复用实盘 CPRP（维持 X:Y = 50:50），每分钟生成多层限价挂单（买/卖各多层）
    2) 撮合：用 1 分钟 K 线的 OHLC（Open/High/Low/Close：开/高/低/收）来模拟“这一分钟里价格怎么走”
       - 阳线（close >= open）：开 → 低 → 高 → 收
       - 阴线（close <  open）：开 → 高 → 低 → 收
       这是在只有 OHLC 的前提下，一个非常常用、且顺序明确的近似方案
    3) 波动率/中心价：使用 VolatilityEngine 同口径的递推（增量更新）
       - EWMA（Exponentially Weighted Moving Average：指数加权移动平均）用递推形式更新，和实盘一致

你要求的“两套执行级回测指标方案”：
    A. 增量口径（单循环）：在撮合主循环里边跑边更新波动率/中心价（更像实盘）
    B. 预计算口径（双循环）：先用同一套递推公式把指标预跑成数组，再把数组喂给撮合（为了性能/参数遍历）
    我们会用测试保证：B 只是更快，不会让策略逻辑和 A 不一致。
...
# ====== 数学内核（回测/实盘同口径）======
from 策略仓库.八号香农策略.program.shannon_math import (
    _可选njit,
    _目标持仓名义,
    _波动率引擎_更新一次,
    _预计算_波动率状态序列,
...
# ====================================================================
# Numba 加速：K线内撮合（挂单更新 + 成交）
# ====================================================================
# 注意：波动率递推 / CPRP 挂单计算已统一抽到 `program/shannon_math.py`
# backtest.py 直接 import 复用，保证回测/实盘同口径。
...
def _执行_单根K线_撮合(
    ...
    ideal_buy_prices: np.ndarray,
    ideal_buy_qtys: np.ndarray,
    ideal_sell_prices: np.ndarray,
    ideal_sell_qtys: np.ndarray,
    buy_prices: np.ndarray,
    buy_qtys: np.ndarray,
    buy_active: np.ndarray,
    sell_prices: np.ndarray,
    sell_qtys: np.ndarray,
    sell_active: np.ndarray,
    last_grid_width: float,
    last_regime: int,
    上一根有成交: bool,
    成交次数: int,
):
    """
    处理一根 K 线（给定本根开始时的指标状态），完成：
    - 计算理想挂单、判断是否需要撤单重挂
    - 用 OHLC 内部路径撮合成交
    - 输出本根收盘权益、网格宽度
...
def _执行级回测_核心循环_增量指标(
    ...
    for idx in range(start, n):
        ...
        out_i = idx - start
        if 爆仓:
            # 剩余曲线置 0（宽度/状态沿用爆仓瞬间的值）
            for j in range(out_i, m):
                权益曲线[j] = 0.0
                宽度曲线[j] = width
                状态曲线[j] = regime
...
def _执行级回测_核心循环_预计算指标(
    ...
    buy_prices = np.zeros(grid_layers, dtype=np.float64)
    buy_qtys = np.zeros(grid_layers, dtype=np.float64)
    buy_active = np.zeros(grid_layers, dtype=np.uint8)
    sell_prices = np.zeros(grid_layers, dtype=np.float64)
    sell_qtys = np.zeros(grid_layers, dtype=np.float64)
    sell_active = np.zeros(grid_layers, dtype=np.uint8)
    ...
    for idx in range(start, n):
        out_i = idx - start
        ewma_vol = ewma_vol_in[out_i]
        ewma_price = ewma_price_in[out_i]
        regime = int(regime_in[out_i])
...
def 运行回测(显示图表: bool = True, 限制条数: int | None = None, 指标模式: str = "incremental") -> None:
    """
    指标模式：
        - incremental：方案 A，增量口径（单循环）
        - precompute：方案 B，预计算口径（双循环）
    """
    try:
        print()
        print("🚀" * 20)
        ...
        # ====== 3. 执行回测（执行级撮合）======
        开 = df['open'].to_numpy(dtype=np.float64)
        高 = df['high'].to_numpy(dtype=np.float64)
        低 = df['low'].to_numpy(dtype=np.float64)
        收 = df['close'].to_numpy(dtype=np.float64)
        时间 = df['candle_begin_time'].to_numpy()

        # Numba 更喜欢连续内存（速度更稳定）
        开 = np.ascontiguousarray(开)
        高 = np.ascontiguousarray(高)
        低 = np.ascontiguousarray(低)
        收 = np.ascontiguousarray(收)

        logger.info(f"⚡ 开始执行级回测 | 指标模式={指标模式} | 数据量: {len(df):,} 条 | 交易起点index={交易起始索引}")
        ...
        grid_layers = int(getattr(配置, "grid_layers", 3))
        force_order_band = float(getattr(配置, "force_order_band", 0.1))
        min_qty = float(getattr(配置, "min_qty", 0.007))
        update_threshold_ratio = float(getattr(配置, "update_threshold_ratio", 0.05))

        if 指标模式 == "precompute":
            ewma_vol_in, ewma_price_in, regime_in = _预计算_波动率状态序列(
                收,
                int(交易起始索引),
            ...
            权益曲线, 宽度曲线, 状态曲线, 成交次数 = _执行级回测_核心循环_预计算指标(
                开,
                高,
                低,
                收,
            ...
            权益曲线, 宽度曲线, 状态曲线, 成交次数 = _执行级回测_核心循环_增量指标(
                开,
                高,
                低,
                收,
        ...
        回测天数 = 指标结果.回测天数 if 指标结果.回测天数 > 0 else 1
        日均成交次数 = 成交次数 / 回测天数
        print(f"   日均成交次数: {日均成交次数:.1f}")
        ...
        回测配置参数 = {
            ...
            "grid_layers": int(grid_layers),
            "force_order_band": float(force_order_band),
            "min_qty": float(min_qty),
            "update_threshold_ratio": float(update_threshold_ratio),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="incremental",
        choices=["incremental", "precompute"],
        help="指标口径：incremental=增量口径(单循环)，precompute=预计算口径(双循环)",
    )
    args = parser.parse_args()

    运行回测(显示图表=not args.no_chart, 限制条数=args.limit, 指标模式=args.indicator_mode)
