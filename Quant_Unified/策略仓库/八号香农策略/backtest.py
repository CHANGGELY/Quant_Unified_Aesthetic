...
def _可选njit(*args, **kwargs):
    """
    一个“小开关”：
    - 如果环境里有 Numba（把 Python 循环即时编译成机器码的库），就用 njit 加速
    - 如果没有，也能跑，只是会慢很多
    """
...
def _预计算_波动率状态序列(
    ...
    """
    方案 B（预计算口径）：
        先把“每根 K 线开始时”的 (ewma_vol, ewma_price, regime) 预跑出来，存成数组。

    注意：
        - 这里用的递推公式与“增量口径”完全同一套（通过共享 _波动率引擎_更新一次 实现）
        - 输出序列长度 = n - start，对齐回测输出曲线长度
    """
    n = len(close_arr)
    start = 交易起始索引
    if start < 2:
    ...
    for idx in range(start, n):
        k = idx - start
        ewma_vol_in[k] = ewma_vol
        ewma_price_in[k] = ewma_price
        regime_in[k] = regime
