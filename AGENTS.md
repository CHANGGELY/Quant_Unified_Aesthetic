...
*   **中文命名**：函数名、变量名、类名**尽可能使用中文**。
    ...
    *   **例外（务必遵守）**：除以下情况外，其余地方尽可能中文命名：
        1) **Python 模块/包名**（会被 `import` 的文件/目录名），尽量保持稳定且少改（改名会牵连全仓库 import）
        2) **对中文路径兼容一般的少数工具链/第三方依赖所依赖的路径**（例如某些构建脚本、CI、容器构建等）
        3) **底层公共库/对外入口**（例如统一入口脚本、对外 API/CLI 的稳定路径），优先稳定性而不是“看起来更中文”
...
*   **错误自愈**：编写前端自动化测试或脚本时，自动调用 `chrome dev tools` MCP 修复报错。
